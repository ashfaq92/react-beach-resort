...
class RoomProvider extends Component {
    ...
    formatData(items) {
        let tempItems = items.map(item => {
            let id = item.sys.id
            let images = item.fields.image.map(image => {
    ...
    getRoom = slug => {
        ...
        const targetRoom = tempRooms.find(room => room.slug === slug)
        return targetRoom
    }
    handleChange = event => {
    ...
    filterRooms = () => {
        ...
        // filter by size
        tempRooms = tempRooms.filter(item => minSize <= item.size && item.size <= maxSize)
        // filter by breakfast
        if (breakfast)
